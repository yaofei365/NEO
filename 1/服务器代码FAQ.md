...
(1) dbmgr 与 centermgr 在启动进程时, 会将 `mysql` 加载至"内存数据库" (以下相关表的数据不会加载至"内存数据库")    
a) 表名 `tb_ad_` 开头的表(通常是 php 相关表)      
b) 表名 `tb_rp_` 开头的表(通常是 php 相关表)    
c) 表名 `tmp_` 开头的表(通常是处理某个问题的临时表 )
d) `vw_` 开头的视图    
(2) 进程启动后, 一切数据更新都先更新至"内存数据库"，再定时同步至"mysql";     
(3) 基于 (2) 的描述, 不建议在进程启动时直接修改 mysql; 通常是停止进程后, 在 mysql 作相关数据修改, 再重新启动进程;     
